...
// ==========================================================================
// Formal verification of UncheckedExtrinsic preamble encoding format
// ==========================================================================
//
// UncheckedExtrinsic wire format:
//   compact_encode(payload_len) ++ preamble_byte ++ preamble_data ++ call_bytes
//
// Where preamble_byte = extrinsic_version | preamble_type
//   - VERSION_MASK = 0x3F extracts version (lower 6 bits)
//   - TYPE_MASK    = 0xC0 extracts type (upper 2 bits)
//
// Preamble variants:
//   Bare(v)               → [v | 0x00]  (versions 4, 5)
//   Signed(addr, sig, ext)→ [4 | 0x80] ++ encode(addr) ++ encode(sig) ++ encode(ext)
//   General(ext_v, ext)   → [5 | 0x40] ++ encode(ext_v) ++ encode(ext)

/// Spec: combine extrinsic version (lower 6 bits) with preamble type (upper 2 bits)
pub open spec fn spec_encode_version_type(version: u8, ptype: u8) -> u8 {
	version | ptype
}

/// Spec: extract extrinsic version from combined byte (lower 6 bits)
pub open spec fn spec_decode_version(byte: u8) -> u8 {
	byte & 0x3Fu8
}

/// Spec: extract preamble type from combined byte (upper 2 bits)
pub open spec fn spec_decode_preamble_type(byte: u8) -> u8 {
	byte & 0xC0u8
}

// -------------------------------------------------------------------
// Core theorem: preamble version/type byte encoding is bijective
// -------------------------------------------------------------------

/// For any valid version (≤ 63) and preamble type (Bare/Signed/General),
/// encoding then decoding the combined byte recovers both values.
pub proof fn theorem_preamble_byte_roundtrip(version: u8, ptype: u8)
	requires
		version <= 63u8,
		ptype == 0x00u8 || ptype == 0x80u8 || ptype == 0x40u8,
	ensures
		spec_decode_version(spec_encode_version_type(version, ptype)) == version,
		spec_decode_preamble_type(spec_encode_version_type(version, ptype)) == ptype,
{
	assert(((version | ptype) & 0x3Fu8) == version) by(bit_vector)
		requires version <= 63u8, ptype == 0x00u8 || ptype == 0x80u8 || ptype == 0x40u8;
	assert(((version | ptype) & 0xC0u8) == ptype) by(bit_vector)
		requires version <= 63u8, ptype == 0x00u8 || ptype == 0x80u8 || ptype == 0x40u8;
}

/// Bare preamble byte is the version itself (OR with 0x00 is identity).
pub proof fn theorem_bare_preamble_identity(version: u8)
	requires version == 4u8 || version == 5u8
	ensures
		spec_encode_version_type(version, 0x00u8) == version,
		spec_decode_version(version) == version,
		spec_decode_preamble_type(version) == 0x00u8,
{
	assert((version | 0x00u8) == version) by(bit_vector);
	assert((version & 0x3Fu8) == version) by(bit_vector)
		requires version == 4u8 || version == 5u8;
	assert((version & 0xC0u8) == 0x00u8) by(bit_vector)
		requires version == 4u8 || version == 5u8;
}

/// Signed preamble byte is 0x84. Decoding recovers version=4, type=SIGNED.
pub proof fn theorem_signed_preamble_byte()
	ensures
		spec_encode_version_type(4u8, 0x80u8) == 0x84u8,
		spec_decode_version(0x84u8) == 4u8,
		spec_decode_preamble_type(0x84u8) == 0x80u8,
{
	assert((4u8 | 0x80u8) == 0x84u8) by(bit_vector);
	assert((0x84u8 & 0x3Fu8) == 4u8) by(bit_vector);
	assert((0x84u8 & 0xC0u8) == 0x80u8) by(bit_vector);
}

/// General preamble byte is 0x45. Decoding recovers version=5, type=GENERAL.
pub proof fn theorem_general_preamble_byte()
	ensures
		spec_encode_version_type(5u8, 0x40u8) == 0x45u8,
		spec_decode_version(0x45u8) == 5u8,
		spec_decode_preamble_type(0x45u8) == 0x40u8,
{
	assert((5u8 | 0x40u8) == 0x45u8) by(bit_vector);
	assert((0x45u8 & 0x3Fu8) == 5u8) by(bit_vector);
	assert((0x45u8 & 0xC0u8) == 0x40u8) by(bit_vector);
}

/// Each valid preamble byte decodes to exactly one variant — no ambiguity.
pub proof fn theorem_preamble_byte_unique(byte: u8)
	ensures
		(spec_decode_preamble_type(byte) == 0x00u8 &&
			(spec_decode_version(byte) == 4u8 || spec_decode_version(byte) == 5u8))
			==> (byte == 4u8 || byte == 5u8),
		(spec_decode_preamble_type(byte) == 0x80u8 && spec_decode_version(byte) == 4u8)
			==> byte == 0x84u8,
		(spec_decode_preamble_type(byte) == 0x40u8 && spec_decode_version(byte) == 5u8)
			==> byte == 0x45u8,
{
	assert(
		(byte & 0xC0u8) == 0x00u8 &&
		((byte & 0x3Fu8) == 4u8 || (byte & 0x3Fu8) == 5u8)
		==> (byte == 4u8 || byte == 5u8)
	) by(bit_vector);
	assert(
		(byte & 0xC0u8) == 0x80u8 && (byte & 0x3Fu8) == 4u8
		==> byte == 0x84u8
	) by(bit_vector);
	assert(
		(byte & 0xC0u8) == 0x40u8 && (byte & 0x3Fu8) == 5u8
		==> byte == 0x45u8
	) by(bit_vector);
}

/// The four valid preamble bytes (0x04, 0x05, 0x84, 0x45) are all distinct.
pub proof fn theorem_preamble_bytes_disjoint()
	ensures
		4u8 != 5u8 && 4u8 != 0x84u8 && 4u8 != 0x45u8,
		5u8 != 0x84u8 && 5u8 != 0x45u8,
		0x84u8 != 0x45u8,
{
}
